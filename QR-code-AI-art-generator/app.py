...
def get_default_pipe():
    global default_pipe
    if default_pipe is None:
        default_pipe = create_pipe()
    return default_pipe


def create_pipe(base_model: str="runwayml/stable-diffusion-v1-5",
...
def inference(
    ...
    if init_image and use_qr_code_as_init_image:
        raise gr.Error("Both init image and use QR code as init image are"
                       " selected. Please select only one.")

    _pipe.scheduler = SAMPLER_MAP[sampler](_pipe.scheduler.config)
...
def start_gradio_ui(share=True, use_queue=False):
